...
extern uint16_t temperature_dataset[][HC3D_TM_CONFIG_TEMP_SENSOR_COUNT];

// Wraps around at HC3D_TM_TEMPERATURE_DATASET_SIZE
...
void driver_temp_read(uint16_t* arr, uint8_t length){
	if(unit_test_data_frame == HC3D_TM_TEMPERATURE_DATASET_SIZE-1){
		// Wrap around frame counter
		unit_test_data_frame = 0;
	}
	
	// Copy data from dataset table into supplied array
	memcpy(arr, temperature_dataset[unit_test_data_frame], HC3D_TM_CONFIG_TEMP_SENSOR_COUNT);
